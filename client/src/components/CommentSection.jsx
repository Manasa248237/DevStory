...
export default function CommentSection({ articleId, articleAuthorId }) {
  ...
  return (
    <section className="space-y-8 pt-8 border-t border-slate-200 dark:border-slate-800" aria-label="Comments section">
      {/* Section Header */}
      ...
      {isAuthenticated ? (
        <form onSubmit={handleCreateComment} className="p-5 sm:p-6 bg-slate-50/80 dark:bg-slate-900/80 rounded-2xl border border-slate-200/80 dark:border-slate-800 space-y-4 shadow-2xs">
          <div className="flex items-center justify-between">
            <label htmlFor="newComment" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              Leave a Comment
            </label>
            <span className={`text-xs font-mono font-medium ${newContent.length > 1000 ? "text-rose-600 font-bold" : "text-slate-400 dark:text-slate-500"}`}>
          ...
          {submitError && (
            <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-rose-700 dark:text-rose-400 text-xs font-medium">
              {submitError}
            </div>
          )}
          ...
          <textarea
            ...
            className="w-full p-3.5 text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all"
          />

          <div className="flex items-center justify-between pt-1">
            <div className="flex items-center gap-2">
              {user?.avatar ? (
                <img src={user.avatar} alt={user.name} className="w-6 h-6 rounded-full object-cover border border-slate-200 dark:border-slate-700" />
              ) : (
              ...
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                Commenting as <strong className="text-slate-900 dark:text-white">{user?.name}</strong>
              </span>
            </div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-indigo-50/70 via-violet-50/70 to-purple-50/70 dark:from-indigo-950/40 dark:via-purple-950/40 dark:to-slate-900/60 border border-indigo-100 dark:border-indigo-900/50 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left shadow-2xs">
